...
import com.dancingLink.algo.DLX;
import com.dancingLink.algo.DLXResult;
import com.dancingLink.algo.DLXResultProcessor;
...
public class DLXSudokuSolver  {
	 ...
	 */
	public PuzzleStatusCheck createPuzzle(byte[] puzzle, List<DataObject> solution, int maxSolTocheck) {
		PuzzleStatusCheck puzzleStatus = new PuzzleStatusCheck();
		if (!SudokuUtils.isPuzzleLegal(puzzle)) {
			puzzleStatus.legal = false;
			return puzzleStatus;	
		}
		puzzleStatus.legal = true;	
		if (SudokuUtils.isPuzzleComplete(puzzle)) {
			puzzleStatus.complete = true;
			return puzzleStatus;
		}
		...
		solve(puzzle, resultProcessor, solution, puzzleStatus ,  maxSolTocheck);
		puzzleStatus.solSize = resultProcessor.getNumSolutions();
		return puzzleStatus;
	...
	public final void solve(byte[] puzzle,
			DLXResultProcessor processor, List<DataObject> solution, PuzzleStatusCheck puzzleStatus , int maxSolTocheck) {
		final ColumnObject h = buildSparseArrayForPuzzle(puzzle);
		
		
		DLX.solvePuzzle(h, true, processor, 0, solution, puzzleStatus ,  maxSolTocheck);
	}
	...
	final ColumnObject buildSparseArrayForPuzzle(byte[] puzzle) {
		List<int[]> rowColValue = new ArrayList<int[]>();
		final byte[][] constraintSet = buildConstraintSets(puzzle, rowColValue);//Put 1 at the constraint that are satisfied or can be satisfied with the given puzzle
		final ColumnObject obj = DLX.buildSparseMatrix(constraintSet,rowColValue, LABELS);
	 ...
	 */
	final byte[][] buildConstraintSets(byte[] puzzle, List<int[]> rowColValue) {
		final List<byte[]> constraintSets = new ArrayList<byte[]>(724);
		for (int j = 0; j < 9; ++j) {
			for (int i = 0; i < 9; ++i) {
				final byte curr = puzzle[j*9+i];
				if (curr > 0) {
					// Cell is already occupied by a 'given'- Constraints already satistied
					final byte[] constraintSet = buildConstraintSet(j, i, (curr-1));
					constraintSets.add(constraintSet);
					int [] cellValue = {j+1,i+1,curr};
					rowColValue.add(cellValue);
				} else {
					// Generate all possibilities for this cell - constraint that possible can be satisfied
					for (int val = 1; val <= 9; ++val) {
						if (isMoveLegal(puzzle, j, i, val)) {
							final byte[] constraintSet = buildConstraintSet(j, i, (val-1));
							constraintSets.add(constraintSet);
							int [] cellValue = {j+1,i+1,val};
							rowColValue.add(cellValue);
						}
					}
				}
	...
	final byte[] buildConstraintSet(int row, int column, int value) {
		...
		buffer[((row*9)+column)] = 1; //- some no at row * column =cell
